...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["member_mail"]))){
        $username_err = "Please enter your username.";
    } else{
        $username = trim($_POST["member_mail"]);
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT member_id,member_mail,member_pass FROM members WHERE member_mail = ?";
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if($password==$hashed_password){
                            ...
                            $_SESSION["member_mail"] = $username;                            
                            
                            
                            header("location:userwelcome.php?member_id=".$_SESSION["member_id"]);
          ...
          <div class="row">
            <div class="col-md-9 col-lg-8 mx-auto">
              ...
              <h5 class="login-heading mb-4 text-center">
                        <a href="userlogin.php" id="acustom" style="text-decoration: none; " class="cactive">User</a>
                        &nbsp;
                        &nbsp;
                        &nbsp;
                        <a href="adminlogin.php" id="acustom" style="text-decoration: none; ">Admin</a>
              </h5>
              <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="form-label-group">
                  <input type="mail" name="member_mail" value="<?php echo $username; ?>" id="inputEmail" class="form-control" placeholder="Email address" required autofocus>
                  <label for="inputEmail">Username</label>
                  <div class="alert-danger mt-3"><?php echo $username_err; ?></div>
                </div>

                <div class="form-label-group">
                  <input type="password" name="member_pass" value="<?php echo $password; ?>" id="inputPassword" class="form-control" placeholder="Password" required>
